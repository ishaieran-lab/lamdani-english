...
window.fbSignInGoogle = async function() {
    try {
        ...
        if (e.code !== 'auth/popup-closed-by-user') _setErr('שגיאה בכניסה עם Google');
    }
};

// ── Email Submit ─────────────────────────────────────────────────
window.fbSubmitEmail = async function() {
...
function _buildModal() {
    ...
    d.innerHTML =
        '<div style="background:#fff;border-radius:1.2rem;padding:2rem 2rem 1.5rem;width:min(92vw,380px);position:relative;text-align:center;font-family:inherit;" onclick="event.stopPropagation()">' +
            ...
            '<button id="fbSubmitBtn" onclick="fbSubmitEmail()" style="width:100%;padding:0.75rem;background:#2563eb;color:#fff;border:none;border-radius:0.6rem;font-size:1rem;font-weight:700;cursor:pointer;margin-bottom:0.9rem;" onmouseover="this.style.background=\'#1d4ed8\'" onmouseout="this.style.background=\'#2563eb\'">הרשמה ←</button>' +
